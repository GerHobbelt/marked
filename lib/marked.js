...
var block = {
  ...
  blockquoteParagraph: /^((?:[^\n]+\n?(?!hr|heading|lheading|blockquote|tag|math))+)\n*/,
  text: /^[^\n]+/,
  math: /^ *(\${2,}) *([\s\S]+?)\s*\1 *(?:\n+|$)/
};
...
block.blockquoteParagraph = replace(block.blockquoteParagraph)
  ('hr', block.hr)
  ('heading', block.heading)
  ('lheading', block.lheading)
  ('blockquote', block.blockquote)
...
block.gfm.blockquoteParagraph = replace(block.blockquoteParagraph)
  ('(?!', '(?!'
    + block.gfm.fences.source.replace('\\1', '\\2') + '|'
    + block.list.source.replace('\\1', '\\3') + '|')
  ();
...
function Lexer(options) {
  ...
  this.tokens.links = this.options.linksCollector || {};
...
Lexer.prototype.token = function(src, top, bq) {
  ...
  var links = {}
    , paragraphRule = bq
      ? this.rules.blockquoteParagraph
  ...
  function uniqueAnchor(text) {
    var rawAnchor = encodeURI(text.toLowerCase().replace(/[\s\r\n]+/g, '-')).replace(/[^\w%]+/g, '-');
...
Renderer.prototype.code = function(code, lang, escaped) {
  ...
  if (!lang) {
    return '<pre><code' + (this.options.highlight ? ' class="hljs"' : '') + '>'
      + (escaped ? code : escape(code, true))
      + '\n</code></pre>';
  }

  return '<pre><code class="'
    + (this.options.highlight ? 'hljs ' : '')
...
function marked(src, opt, callback) {
  if (callback || typeof opt === 'function') {
    ...
    try {
      ...
      if (opt.execPrepPhaseOnly) {
        callback(null, null, opt);
        return;
      }
    } catch (e) {
      return callback(e);
...
marked.defaults = {
  ...
  mathjax: true,
  linksInNewTab: false,
  noIndentedCode: false,
  linksCollector: null,         // usercode can set this to a `{}` hash table to receive a list of all reflink definitions collected thus far.
  execPrepPhaseOnly: false      // TRUE: only collect links, etc.
};
